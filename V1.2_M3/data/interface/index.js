var config = {
  ...
  "event": async()=> {
    ...
    $(".volume-slider").addEventListener("input", (event)=> {
      var range = event.currentTarget.value * 6;
      $(".volume-current-value").innerText = range;
      config.set.anim(range);
      config.id()!= null?  background.send("volume", {"value":range,"tabId": config.id()}) : background.send("volume", {"value":range}); 
      background.send("volume", range);
    });

    $(".speaker").addEventListener("click", async ()=>{
      config.set.anim(0);
      config.id()!= null? background.send("volume", {"value":0,"tabId": config.id()}) : background.send("volume", {"value":range}); 
